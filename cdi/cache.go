...
const (
	// Name of the environment variable that stores custom CDI config directory path
	EnvVariableNameCustomCDIDir = "CDI_CONFIG_DIR"
)
...
func NewCache(options ...Option) (*Cache, error) {
	...
	WithSpecDirs(c.GetSpecDirectories()...)(c)
	c.Lock()
	defer c.Unlock()
...
func (c *Cache) GetSpecDirectories() []string {
	...
	dirs := make([]string, len(DefaultSpecDirs))
	copy(dirs, DefaultSpecDirs)

	envValue, envPresent := os.LookupEnv(EnvVariableNameCustomCDIDir)
	if envPresent {
		dirs = append(dirs, envValue)
	}
